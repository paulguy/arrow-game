...
func _physics_process(_delta : float):
	for i in arrow_map.size.x * arrow_map.size.y * FLY_CHANCE:
		var flypath : Array[Vector2i] = arrow_map.get_random_free_fly(astar)
		if len(flypath) > 0 and flypath[0].x >= 0 and flypath[0].y >= 0:
			tile_map.erase_cell(flypath[0])
	...
	if active_snake != null:
		...
		if (active_snake.headTowards == SIDE_LEFT and pos.x == 0) or \
		   ...
		   (active_snake.headTowards == SIDE_BOTTOM and pos.y == arrow_map.size.y - 1):
			...
			arrow_map.delete_snake_astar(-1, astar)
	...
	if offscreen_snake != null:
		...
		if (offscreen_snake.headTowards == SIDE_LEFT and \
		   (offscreen_snake.pos.x + offscreen_overhang) * tile_map.tile_set.tile_size.x + position.x < 0) or \
		   (offscreen_snake.headTowards == SIDE_RIGHT and \
		   (offscreen_snake.pos.x - offscreen_overhang) * tile_map.tile_set.tile_size.x + position.x > get_viewport_rect().end.x) or \
		   (offscreen_snake.headTowards == SIDE_TOP and \
		   (offscreen_snake.pos.y + offscreen_overhang) * tile_map.tile_set.tile_size.y + position.y < 0) or \
		   (offscreen_snake.headTowards == SIDE_BOTTOM and \
		   (offscreen_snake.pos.y - offscreen_overhang) * tile_map.tile_set.tile_size.y + position.y > get_viewport_rect().end.y):
			# went offscreen
			arrow_map.snakes.append(offscreen_snake)
			arrow_map.delete_snake_tilemap(-1, tile_map)
			arrow_map.snakes.pop_back()
			offscreen_snake.free()
			offscreen_snake = null
		else:
			arrow_map.snakes.append(offscreen_snake)
			arrow_map.move_snake_tilemap(-1, offscreen_snake.headTowards, tile_map)
			arrow_map.snakes.pop_back()
